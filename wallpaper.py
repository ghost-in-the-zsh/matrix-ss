...
def _impl_winblows() -> Text:
    '''Get the wallpaper path in Winblows, if we can find it :|

    This implementation is a "best-effort" and is *not* guaranteed to
    work. The `winreg` code was tested to work in Windows 7 (Python 3.7)
    and 10 (Python 3.11). However, there *are* some details to keep in
    mind:
    ...
    1. In Win7, when an image is set as the wallpaper, it gets copied over
        ...
        suggesting that we should always be able to find it.

    2. In Win10, the same tuple structure as in Win7 is returned, but the
        path within it MAY NOT BE VALID. What you get is the location
        where the image used to be AT THE TIME IT WAS SET as the wallpaper
        by the user. By the time we `QueryValueEx`, the image could've
        been moved, deleted, renamed, etc. It needs an extra check to
        verify whether the file actually exists or not, hen the
        `open(...)` call.
